...
{	
	...
	while(1)
	{
		...
		{	case CREATE:		//for create,type=1,data2=inode of current dir,ptr=ptr to path
			...
			{
				CopyFrom(pid,(void *)pname,msg.ptr,MAXPATHNAMELEN);
				msg.type=Server_Open(pname,msg.data2);
				Reply(&msg,pid);
				break;
			}
			...
			case READLINK:
			{
				...
				temp_buff=(char *)malloc(msg_readlink.len);
				msg.type=Server_ReadLink(msg.data2,msg_readlink.pathname,msg_readlink.len,temp_buff);
				CopyTo(pid,msg_readlink.buf,temp_buff,msg_read.len);
				Reply(&msg,pid);
				break;
			}
			case SYMLINK:
			{
				CopyFrom(pid,(void *)&msg_link,msg.ptr,sizeof(struct old_new));
				msg.data2=Server_UnLink(msg.data2,msg_link.oldname,msg_link.newname);
				if(msg.data2==ERROR)
					msg.type=ERROR;
				else
					msg.type=0;
...
int Server_ReadLink(int inum,char *pathname,int len,char *buff)
{
	...
	{	
		...
		for(i=0;i<BLOCKSIZE/sizeof(char);i++)
		{
			if((len_read<len)&&(len_read<curr_inode.size))
			{	
				buff[len_read++]=str[i];
...
int Server_SymLink(int inode_num,char *oldname,char *newname)
{
	...
	curr_inode.nlink=1;
	curr_inode.size=MAXPATHNAMELEN;
	...
	strncpy(&block,oldname,MAXPATHNAMELEN);
	block[MAXPATHNAMELEN]='\0';
...
int stat(int inode_num,char *name, struct Stat *stat_msg)
{
	...
	struct inode curr_inode;
	int res;
	res=Server_Open(name,inode_num);
	if(res==ERROR)
		return ERROR;
...
int Server_Open(char *pathname,int inode_num)
{
	...
	if(inode_num<1||inode_num>max_inode_num||pathname==NULL)
		return ERROR;
	res=LookUp(pathname,inode_num,0);
	if((res==0)||(res==ERROR))	
		return ERROR;
