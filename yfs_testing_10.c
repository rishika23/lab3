...
int max_inode_num;
int max_block_num;
hashtable_t *inode_hashtable;
hashtable_t *block_hashtable;
...
int Is_Dir_Block_Empty(int blk_num,int offset);
...
int Server_Sync()
{
	/*To write all the dirty inodes back to disk*/
	register int i_num;
	int i,sec_num,res,j,k,len,blk_num;
	struct inode curr_inode;
	char *dir=(char *)malloc(SECTORSIZE);
	char curr_block[BLOCKSIZE];
	printf("\nReached 1");
	struct inode *fsp=(struct inode *)malloc(SECTORSIZE);
	...
	i_num=0;
	while(i_num<max_inode_num)
	{	i_num=i_num+1;
		printf("\nReached 2 with val of i_num %d and max_inode_num=%d\n",i_num,max_inode_num);
		res=ht_get_inode(inode_hashtable,i_num,&curr_inode);
		printf("\nReached 2.3\n with res =%d",res);
		if(res==0)
		{	
			sec_num=BLOCKSIZE/SECTORSIZE+((i_num*INODESIZE)/SECTORSIZE);
			i=(i_num*INODESIZE)%SECTORSIZE;
			i=i/INODESIZE;
			
			ReadSector(sec_num,(void *)fsp);
			
			
			strncpy(&fsp[i],&curr_inode,INODESIZE);
			
			WriteSector(sec_num,(void *)fsp);
			printf("\nReached 2.4 \n");
	...
	/*To write all the dirty blocks back to disk
	for(blk_num=1;blk_num<max_block_num;blk_num++)
	{
		//printf("\nReached 3");
		res=ht_get_block(block_hashtable, blk_num,&curr_block);
		if(res==0)
		{	len=0;
			printf("\nReached 3");
...
int GetBlock(int blk_num,char *ptr)
{
	...
	res=ht_get_block(block_hashtable,blk_num, ptr);
	if(res==0)
	{	printf("\nFound block %d",blk_num);
		free(dir);
		return 0;
	}
	...
	printf("\nNot Found block %d",blk_num);

	memcpy(&temp_block,ptr,BLOCKSIZE);
	res=ht_set_block(block_hashtable, blk_num,&temp_block);
	if(res!=0)
	{	len=0;
		blk_num=res;
		sec_num=(blk_num*BLOCKSIZE)/SECTORSIZE;
...
int WriteBlock(int blk_num,char *ptr)
{
	...
	if(blk_num<0 || blk_num>max_block_num||ptr==NULL)
		return ERROR;
	printf("\nIn Write Block %d",blk_num);

	res=GetBlock(blk_num,&temp_block);
	printf("\nBlock before writing was %s",temp_block);
	res=ht_set_block(block_hashtable, blk_num,ptr);
	res=GetBlock(blk_num,&temp_block);
	printf("\nBlock after writing was %s",temp_block);
...
int Server_RmDir(char *pathname,int inode_num)
{
	...
	if(curr_inode.nlink>2)
	{
		...
		size=curr_inode.size;
		total_blocks=(size+BLOCKSIZE-1)/BLOCKSIZE;
		...
		for(i=0;i<total_blocks;i++)
		{
			...
			if(offset>0)
				...
				offset=size%BLOCKSIZE;
			count+=Is_Dir_Block_Empty(blk_num,offset);
			size-=BLOCKSIZE;
		}
		if(count==0)
		{
...
int Is_Dir_Block_Empty(int blk_num,int offset)
{
	...
	if(res==ERROR)
		return ERROR;
	for(k=0;k<(BLOCKSIZE/DIRECTORYSIZE);k++)
	{	printf("\ninum stored in dir= %d with offset=%d",dir[k].inum,curr_offset);
		if(offset<=curr_offset)
			break;
		curr_offset+=DIRECTORYSIZE;
		if(dir[k].inum!=0)
		{		
			for(p=0;p<DIRNAMELEN;p++)
				dname[p]=dir[k].name[p];
			dname[p]='\0';
			printf("\nNot Sure if Incrementing count due to %s",dname);
...
int write_inode(int inode_num,struct inode *curr_inode)
{
	...
	res=GetInode(inode_num,&free_inode);	
	res=ht_set_inode( inode_hashtable, inode_num,curr_inode);
...
int GetInode(int inode_num,struct inode *curr_inode)
{
	...
	res=ht_get_inode(inode_hashtable,inode_num, curr_inode);
	if(res==0)
		return 0;
	...
	memcpy(&new_inode,curr_inode,INODESIZE);

	res=ht_set_inode( inode_hashtable, inode_num,&new_inode);
	if(res!=0)
	{	inode_num=res;
		sec_num=BLOCKSIZE/SECTORSIZE+((inode_num*INODESIZE)/SECTORSIZE);
		i=(inode_num*INODESIZE)%SECTORSIZE;
		...
		if(!res)
		{
			memcpy((fs+i),&new_inode,INODESIZE);
			WriteSector(sec_num,(void *)fs);
		}
...
int SearchInDir(int in_num,char *name,int num_sym_link)
{
	...
	printf("\nBefore GetNode:Printing Inode:");
	print_inode(in_num,cinode);

	GetInode(in_num,cinode);
	printf("\nAfter GetInode Printing Inode:");
	print_inode(in_num,cinode);
...
Initialize_Server()
{
	...
	for(i=num_blocks-1;i>=num_blocks-nfree_block;i--)
		if(free_blocks[i]==1)
			Add_free_blknum(i);
	inode_hashtable = ht_create_inode(INODE_CACHESIZE);
	block_hashtable = ht_create_block(BLOCK_CACHESIZE);
